...
TestUtilities.StartTest(out, "PR Trigger Creation - Branch Specific Default - Legacy")
...
Utilities.addGithubPRTriggerForBranch(newJob, branchName, 'PR Context')
...
assert JobReport.Report.prTriggeredJobs[newJob.name].originalTriggerPhrase == '(?i).*test\\W+PR Context.*'
assert JobReport.Report.prTriggeredJobs[newJob.name].triggerPhrase == 'test PR Context'
