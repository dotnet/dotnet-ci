...
repos.each { repoInfo ->
    ...
    [true, false].each { isPRTest ->
        ...
        if (isPRTest) {
            ...
            Utilities.addPrivateGithubPRTriggerForBranch(jobGenerator, repoInfo.branch, "CI Gen Test for ${repoInfo.branch}/${repoInfo.definitionScript} on ${repoInfo.server}", '(?i).*test\\W+ci.*', ['Microsoft'], null)
        }
        else {
